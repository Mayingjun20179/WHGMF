...
if cv_flag==1
    ...
    best_HWGMF_CVa = result;
    save best_HWGMF_CVa best_HWGMF_CVa;
elseif cv_flag==2
    for j = 1:length(seed)
    ...
    best_HWGMF_CVr = result;
    save best_HWGMF_CVr best_HWGMF_CVr;
elseif cv_flag==3
    for j = 1:length(seed)
        cv_data = cross_validation(intMat,cv_flag,cv,1,seed(j));
        result0 = five_cross(DATA,cv_data,option);
    ...
    best_HWGMF_CVc = result;
    save best_HWGMF_CVc best_HWGMF_CVc;
end
...
for k=1:cv
    ...
    scores = HWGMF_opt(W,train_set, drug_sim,micro_sim,option);
