...
if cv_flag==1
    ...
    best_HGRGMF_CVa = result;
    save best_HGRGMF_CVa best_HGRGMF_CVa;
elseif cv_flag==2
    for j = 1:length(seed)
    ...
    best_HGRGMF_CVr = result;
    save best_HGRGMF_CVr best_HGRGMF_CVr;
elseif cv_flag==3
    for j = 1:length(seed)
        cv_data = cross_validation(intMat,cv_flag,cv,1,seed(j));
        result0 = five_cross(DATA,cv_data,option);
    ...
    best_HGRGMF_CVc = result;
    save best_HGRGMF_CVc best_HGRGMF_CVc;
end
...
for k=1:cv
    ...
    K1(:,:,1) =  DATA.drug_sim{1};  K1(:,:,2) =  KSNS_Sdrug;
    K1 = Assist_method_Utils.complete_sim(K1);
    [weight_v1] = Assist_method_Utils.cka_kernels_weights(K1,train_set,1);
    drug_sim = Assist_method_Utils.combine_kernels(weight_v1, K1);
    
    %%%计算疾病的相似矩阵(并利用高斯核融合）
    KSNS_Smicro = Assist_method_Utils.KSNS_opt(Y');     KSNS_Smicro(1:size(KSNS_Smicro,1)+1:end)=1;
    K2(:,:,1) =  DATA.micro_sim{1};  K2(:,:,2) =  DATA.micro_sim{2};  K2(:,:,3) =  KSNS_Smicro;
    K2 = Assist_method_Utils.complete_sim(K2);
    [weight_v2] = Assist_method_Utils.cka_kernels_weights(K2,train_set,2);
    micro_sim = Assist_method_Utils.combine_kernels(weight_v2, K2);
    scores = GRGMF_opt(W,train_set, drug_sim,micro_sim,option);
