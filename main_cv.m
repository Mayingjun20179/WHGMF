...
if cv_flag==1
    ...
    best_WHGMF_CVa = result;
    save best_WHGMF_CVa best_WHGMF_CVa;
elseif cv_flag==2
    for j = 1:length(seed)
    ...
    best_WHGMF_CVr = result;
    save best_WHGMF_CVr best_WHGMF_CVr;
elseif cv_flag==3
    for j = 1:length(seed)
        cv_data = cross_validation(intMat,cv_flag,cv,1,seed(j));
        result0 = five_cross(DATA,cv_data,option);
    ...
    best_WHGMF_CVc = result;
    save best_WHGMF_CVc best_WHGMF_CVc;
end
...
for k=1:cv
    ...
    scores = WHGMF_opt(W,train_set, drug_sim,micro_sim,option);
